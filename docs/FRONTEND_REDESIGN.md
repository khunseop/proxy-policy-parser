...
| 항목 | 현재 결정 | 대안 |
|------|---------|------|
| Path depth 표현 | 들여쓰기(padding) | 별도 depth 컬럼 |
| 그룹 행 펼침/접기 | 미구현 (flat 유지) | 클릭으로 하위만 표시/숨김 |
| 상세 패널 위치 | 오른쪽 슬라이드 | 하단 drawer / 모달 |
| 정책 전체 로드 방식 | 초기 1회 전체 로드 | 무한 스크롤 / 페이지네이션 |
| 기존 Vanilla JS 유지 기간 | Phase 4까지 병행 | 즉시 제거 |
