...
class SkyhighSWGClient:
    ...
    def logout(self):
        ...
        try:
            ...
            if response.ok:
                ...
                logger.warning(f"로그아웃 실패: {response.status_code} {response.text}")
    ...
    def fetch_ruleset_xml(self, ruleset_id):
        """Rule Set XML 데이터를 직접 반환 (파싱 로직과 분리)"""
        url = self._build_url(f'rulesets/rulegroups/{ruleset_id}/export')
        response = self.session.post(url, verify=self.verify_ssl)
        if response.ok:
            return response.content
        else:
            raise Exception(f"Rule Set {ruleset_id} XML 데이터 가져오기 실패: {response.status_code} {response.text}")
